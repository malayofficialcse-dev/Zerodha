...
const getAllHoldings = async (req, res) => {
  try {
    ...
    // Ensure user has some holdings seeded so the dashboard displays actual data
    const allData = await ensureUserHoldings(userId);
    ...
    try {
      const allData = await ensureUserHoldings(req.user.userId);
      res.json(allData);
    } catch (dbErr) {
